...
namespace Alpha_API.Controllers
{
	...
	public class PackagesAndTrainersController : ControllerBase
	{
		...
		public async Task<IActionResult> GetPackagesAndTrainers([FromQuery] string type)
		{
			...
			{
				var boxingPackagesTask = _firebaseClient
					.Child("BoxingOptions")
					.OnceAsync<BoxingOption>();

				// Lấy danh sách PT
				var allTrainersTask = _firebaseClient
					.Child("Trainers")
					.OnceAsync<Trainer>();

				await Task.WhenAll(boxingPackagesTask, allTrainersTask);

				var boxingPackages = boxingPackagesTask.Result;

				var allTrainers = allTrainersTask.Result;
			...
			{
				var rentalPackagesTask = _firebaseClient
					.Child("RentalOptions")
				...
				await Task.WhenAll(rentalPackagesTask, allTrainersTask);

				var rentalPackages = rentalPackagesTask.Result;
